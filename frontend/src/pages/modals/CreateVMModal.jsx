...
export default function CreateVMModal({ open, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setName(''); setDesc(''); setSelectedTemplate(null); setImage(IMAGES[0]);
    setCustom({ cpu: 2, ram: 4, disk: 40 });
    onClose();
  };
    ...
    <div className="modal-overlay open" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-lg">
        <div className="modal-header">
          <div>
            <div className="modal-title" style={{ fontSize: 20 }}>Создать виртуальную машину</div>
            <div className="modal-sub">Создание через асинхронную очередь задач</div>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            ...
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: 20, marginBottom: 8 }}>
              <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 14, color: '#374151' }}>⚙️ Ресурсы виртуальной машины</div>
            ...
            {myTenant && (
              <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '12px 16px', fontSize: 14, color: '#1d4ed8' }}>
          ...
          <div className="modal-footer" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 11, color: '#9ca3af', fontFamily: 'monospace' }}>Idempotency-Key: idem-{idKey}</span>
            <div style={{ display: 'flex', gap: 10 }}>
              <button type="button" className="btn btn-ghost" onClick={onClose}>Отмена</button>
              <button type="submit" className="btn btn-primary" disabled={loading} style={{ fontSize: 15, padding: '10px 24px' }}>
